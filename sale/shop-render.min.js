...
async function populateArtistFilter() {
  ...
  try {
    ...
    allArtists.forEach(artist => {
      if (!artist.slug) return; // skip artists without a slug
      const option = document.createElement('option');
      // Use slug as filter value — works for any artist, no hardcoded map needed.
      // slugToId kept below for legacy URL ?id= param compatibility only.
...
function renderAllItems(artworksData) {
  ...
  const normalize = a => {
    ...
    // Use slug directly as the filter key — matches option.value in artistFilter.
    const artist = typeof a.artist === 'object'
      ? (a.artist?.slug || '')
      : (slugToId[a.artist] || a.artist || ''); // legacy data.js IDs mapped back to slug via reverse lookup
